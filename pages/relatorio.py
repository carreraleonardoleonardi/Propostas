...
Relatório consolidado local: LM + Salesforce (Google Sheets) + Cadastro Manual

Fontes:
  - LM:       Dados/base_consolidada_completa.xlsx  (local)
  - Cockpit:  Dados/Cockpit_Carrera.xlsx            (local)
  - Manuais:  Dados/pedidos_manuais.xlsx            (local)
  - Salesforce: Google Sheets (atualiza a cada 4h)  (remoto)
"""
...
SHEET_ID_SALESFORCE = "1RTm8TYpHJPGJKlh7N9AAf6iP212F7Z9u9P_-VdKfT1w"
...
@st.cache_resource
def _gc():
    """Cliente gspread autenticado via secrets.toml."""
...
def _tipo_cliente(doc: str) -> str:
    ...
    if len(digits) == 11: return "PF"
    if len(digits) == 14: return "PJ"
    return ""
...
def _formatar_datas_para_exibicao(df: pd.DataFrame, colunas: list) -> pd.DataFrame:
    dv = df.copy()
    for col in colunas:
...
def _lookup_consultor(df_sf: pd.DataFrame, numero_pedido) -> str:
    if df_sf.empty: return "Não encontrado"
...
def _coluna_data_filtro(df: pd.DataFrame):
    if "dateLastUpdated" in df.columns: return "dateLastUpdated"
    if "Data de Status"  in df.columns: return "Data de Status"
    return None
...
def _preparar_exibicao(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["Mensalidade","Valor Total","Comissão Carrera","Comissão Vendedor R$","KickBack","Preço NF"]:
        if col in dv.columns: dv[col] = _to_num_col(dv, col)
    for col in ["Km","Período","ANO DA VENDA","MÊS DA VENDA"]:
        if col in dv.columns: dv[col] = _to_num_col(dv, col)
    dv = _formatar_datas_para_exibicao(dv, [
        "Data de Inclusão","Data de Status","dateLastUpdated",
        "Data da retirada","Vigência Final do contrato","Data Assinatura",
    ])
    ordem = [
        "Pedido","Locadora","Segmento","Status","Venda","Nome","Documento","Tipo",
        "Marca","Modelo Oficial","Cor","Placa","Chassi",
        "Mensalidade","Valor Total","Preço NF","Comissão Carrera","Comissão Vendedor R$",
        "Período","Km","dateLastUpdated","Data de Inclusão","Data Assinatura",
        "Data de Status","Data da retirada","Vigência Final do contrato",
...
def _top_3_carros_podium(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty: return pd.DataFrame(columns=["modelo","qtd"])
    base = df.copy()
    if "Status" in base.columns:
        base = base[base["Status"].astype(str).str.strip().str.lower() == "pedido concluído"]
    if base.empty: return pd.DataFrame(columns=["modelo","qtd"])
    ...
    if "Marca" in base.columns:
        base["modelo"] = base.apply(
            lambda r: r["modelo"] if str(r["modelo"]).strip() not in ["","nan","None"]
            else str(r.get("Marca","")).strip(), axis=1)
    base["modelo"] = base["modelo"].replace(["","nan","None"], pd.NA)
    base = base.dropna(subset=["modelo"])
    if base.empty: return pd.DataFrame(columns=["modelo","qtd"])
    return (base.groupby("modelo", as_index=False).size()
...
def _render_podium_card(posicao: int, modelo: str, qtd: int) -> str:
    ...
    return f"""
        <div class="podium-col podium-{posicao}">
...
# LEITURA DAS BASES
# ══════════════════════════════════════════════════════════════════════════════
@st.cache_data(ttl=240, show_spinner=False)   # mesmo TTL do Sheets (4h)
def _ler_salesforce() -> pd.DataFrame:
    """
    Lê o Salesforce direto do Google Sheets.
    TTL = 240s (4 min) — a planilha atualiza a cada 4h, cache evita chamadas desnecessárias.
    Fallback: se Sheets falhar, tenta ler Base Salesforce.xlsx local.
    """
    # 1. Tenta Google Sheets
    try:
        gc = _gc()
        sh = gc.open_by_key(SHEET_ID_SALESFORCE)
        ...
        st.warning(f"⚠️ Não foi possível ler o Salesforce do Sheets: {e}. Tentando arquivo local...")

    # 2. Fallback: arquivo local
    arq_local = os.path.join(DADOS_DIR, "Base Salesforce.xlsx")
    if os.path.exists(arq_local):
        try:
            return pd.read_excel(arq_local)
        except Exception as e:
            st.warning(f"⚠️ Também não foi possível ler o arquivo local: {e}")
...
def _normalizar_base_principal(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty: return pd.DataFrame(columns=COLUNAS_RELATORIO)
    base = df.copy()
    mapa = {
        "orderId":"Pedido","clientName":"Nome","cpfCnpj":"Documento",
        "orderStatus":"Status","createdAt":"Data de Inclusão","updatedAt":"Data de Status",
        "dateLastUpdated":"dateLastUpdated","totalOrder":"Valor Total","segment":"Segmento",
        "optional":"Opcional","chassis":"Chassi","deliveryPlate":"Placa",
        "monthlyInstallment":"Mensalidade","monthlyKmValue":"Km","deadline":"Período",
        "brand":"Marca","kickback":"KickBack","vehicleValue":"Preço NF",
        "comissao":"Comissão Carrera","comissao_vendedor":"Comissão Vendedor R$",
        "dealerDelivery":"Local de Venda","estado":"UF","city":"City",
        "orderDate":"Data Assinatura","deliveryDate":"Data da retirada",
        "salesChannel":"Origem venda","clientType":"Tipo","consultorNome":"Venda",
    }
    ...
    if "Vigência Final do contrato" not in base.columns:
        if "Data Assinatura" in base.columns and "Período" in base.columns:
            ...
            base["Vigência Final do contrato"] = base.apply(
                lambda r: (r["Data Assinatura"] + pd.DateOffset(months=int(periodo_num.loc[r.name])))
...
def _enriquecer_com_salesforce(df: pd.DataFrame, df_sf: pd.DataFrame) -> pd.DataFrame:
    ...
    if df_sf.empty or "Nro do Pedido" not in df_sf.columns:
        if "Venda" not in out.columns: out["Venda"] = ""
        return out
    sf_idx = df_sf.copy()
    ...
    if "Venda" not in out.columns:
        ...
        out["Venda"] = out.apply(
            ...
            else _lookup_consultor(df_sf, row.get("Pedido","")), axis=1)
    for col_sf, col_rel in [("Nome da conta","Empresa"),("Origem da venda","Origem venda"),
        ...
        if col_sf in sf_idx.columns:
            out[col_rel] = out.apply(
                lambda row: sf_idx.loc[str(row["Pedido"]).strip(), col_sf]
                if str(row["Pedido"]).strip() in sf_idx.index else row.get(col_rel,""), axis=1)
    return out


def _base_completa_local() -> pd.DataFrame:
    df_base   = _ler_base_local()
    df_sf     = _ler_salesforce()        # ← Google Sheets
    df_manual = _ler_manuais_local()
    partes = []
    if not df_base.empty:   partes.append(_normalizar_base_principal(df_base))
    if not df_manual.empty: partes.append(_garantir_colunas(df_manual, COLUNAS_RELATORIO))
    if not partes: return pd.DataFrame(columns=COLUNAS_RELATORIO)
...
def render():
    ...
    .page-title {{ font-size:28px; font-weight:800; color:{AZUL}; margin:0; padding:0; }}
    .section-title {{ font-size:16px; font-weight:800; color:{AZUL}; margin:0 0 12px 0; }}
    .soft-card {{ background:{FUNDO_CARD}; border:1px solid {BORDA}; border-radius:18px;
                  padding:18px 16px 12px 16px; box-shadow:0 4px 18px rgba(33,49,68,.05); margin-bottom:18px; }}
    .kpi-wrap {{ display:grid; grid-template-columns:repeat(6, minmax(120px, 1fr));
                 gap:12px; margin:18px 0 12px 0; }}
    .kpi-box {{ background:#fff; border:1px solid {BORDA}; border-top:4px solid {DOURADO};
                border-radius:18px; padding:18px 12px 14px 12px; text-align:center;
                box-shadow:0 4px 18px rgba(181,123,63,.06); }}
    .kpi-n {{ font-size:24px; font-weight:800; line-height:1.1; margin-bottom:6px; word-break:break-word; }}
    .kpi-l {{ font-size:11px; font-weight:700; letter-spacing:.7px; color:#94a3b8; text-transform:uppercase; }}
    .mini-note {{ color:{CINZA_TEXTO}; font-size:13px; margin:6px 0 16px 0; }}
    .podium-card {{ background:#fff; border:1px solid {BORDA}; border-radius:18px; padding:18px;
                    box-shadow:0 4px 18px rgba(33,49,68,.05); height:100%; margin-bottom:18px; }}
    .podium-wrap {{ display:flex; align-items:end; justify-content:center; gap:16px;
                    margin-top:18px; min-height:320px; }}
    .podium-col {{ flex:1; display:flex; align-items:center; justify-content:end;
                   flex-direction:column; max-width:320px; }}
    .podium-badge {{ width:44px; height:44px; border-radius:999px; display:flex;
                     align-items:center; justify-content:center; font-weight:800; font-size:22px;
                     margin-bottom:10px; border:3px solid #fff; box-shadow:0 4px 12px rgba(0,0,0,.08); }}
    .podium-box {{ width:100%; border-radius:18px 18px 10px 10px; display:flex; flex-direction:column;
                   align-items:center; justify-content:center; padding:18px 12px; text-align:center;
                   border:1px solid {BORDA}; }}
    .podium-model {{ font-size:26px; margin-bottom:8px; }}
    .podium-name {{ font-size:24px; font-weight:800; color:{AZUL}; line-height:1.1;
                    margin-bottom:10px; text-transform:uppercase; word-break:break-word; }}
    .podium-qtd {{ font-size:42px; font-weight:800; color:{AZUL}; line-height:1; }}
    .podium-leg {{ font-size:12px; font-weight:700; color:#94a3b8; text-transform:uppercase;
                   letter-spacing:.6px; margin-top:6px; }}
    .podium-1 .podium-badge {{ background:linear-gradient(180deg,#f6d36b,#d89b22); color:white; }}
    .podium-2 .podium-badge {{ background:linear-gradient(180deg,#cfd5df,#8b95a7); color:white; }}
    .podium-3 .podium-badge {{ background:linear-gradient(180deg,#efb083,#c96a27); color:white; }}
    .podium-1 .podium-box {{ background:linear-gradient(180deg,#fff8ea,#f5e3b6); min-height:230px; }}
    .podium-2 .podium-box {{ background:linear-gradient(180deg,#f5f7fb,#dfe5ef); min-height:180px; }}
    .podium-3 .podium-box {{ background:linear-gradient(180deg,#fbf2eb,#efd7c8); min-height:150px; }}
    .podium-note {{ font-size:13px; color:{CINZA_TEXTO}; margin-top:14px; }}
    </style>""", unsafe_allow_html=True)

    abas = st.tabs(
        ["📋 Relatório", "📤 Dados", "➕ Cadastro Manual"] if pode_editar else ["📋 Relatório"]
    )
    ...
    with abas[0]:
        ...
        with h1:
            st.markdown('<p class="page-title">📊 Relatório Consolidado</p>', unsafe_allow_html=True)
        with h2:
        ...
        with f1: flt_loc = st.selectbox("Locadora",  ["Todas"] + sorted([x for x in df["Locadora"].dropna().astype(str).unique() if x]), key="rel_loc")
        with f2: flt_seg = st.selectbox("Segmento",  ["Todos"] + sorted([x for x in df["Segmento"].dropna().astype(str).unique() if x]), key="rel_seg")
        with f3: flt_sta = st.selectbox("Status",    ["Todos"] + sorted([x for x in df["Status"].dropna().astype(str).unique()   if x]), key="rel_sta")
        with f4: flt_ven = st.selectbox("Consultor", ["Todos"] + sorted([x for x in df["Venda"].dropna().astype(str).unique()    if x]), key="rel_ven")

        f5, f6, f7, f8 = st.columns(4)
        with f5: dt_inicio = st.date_input("Data início", value=st.session_state.get("rel_dt_inicio", data_inicio_default), key="rel_dt_inicio")
        with f6: dt_fim    = st.date_input("Data fim",    value=st.session_state.get("rel_dt_fim",    data_fim_default),    key="rel_dt_fim")
        with f7: flt_tipo  = st.selectbox("Tipo", ["Todos","PF","PJ"], key="rel_tipo")
        with f8: s_ped     = st.text_input("Pedido / Chassi / Cliente", key="rel_busca", placeholder="Digite para buscar")
        ...
        with cb2:
            if st.button("🧹 Limpar", use_container_width=True, key="rel_limpar"):
                for k in ["rel_loc","rel_seg","rel_sta","rel_ven","rel_tipo","rel_busca",
                          "rel_flt_ok","rel_cols","rel_ordem_col","rel_ordem_desc",
                          "rel_det_pedido","rel_dt_inicio","rel_dt_fim"]:
        ...
        if flt_tipo != "Todos":  dv = dv[dv["Tipo"]     == flt_tipo]
        if col_data_filtro:
            serie_data = pd.to_datetime(dv[col_data_filtro], errors="coerce", dayfirst=True).dt.date
            if dt_inicio: dv = dv[serie_data >= dt_inicio]
            if dt_fim:    dv = dv[serie_data <= dt_fim]
        if s_ped:
            mask = (
        ...
        st.markdown(f"<p class='mini-note'><b style='color:{AZUL}'>{len(dv)}</b> pedido(s) no resultado atual</p>", unsafe_allow_html=True)
        ...
        if top3.empty:
            ...
            cards = []
            if len(ranking) >= 2: cards.append(_render_podium_card(2, ranking[1]["modelo"], int(ranking[1]["qtd"])))
            if len(ranking) >= 1: cards.append(_render_podium_card(1, ranking[0]["modelo"], int(ranking[0]["qtd"])))
            if len(ranking) >= 3: cards.append(_render_podium_card(3, ranking[2]["modelo"], int(ranking[2]["qtd"])))
            st.markdown(f"""
            <div class="podium-card">
                <div style="font-size:13px;color:{CINZA_TEXTO};margin-bottom:8px;">
                    Considerando apenas status: <b>Pedido concluído</b></div>
                <div class="podium-wrap">{"".join(cards)}</div>
                <div class="podium-note">Ranking baseado na quantidade de pedidos com status
                <b>Pedido concluído</b> dentro do resultado filtrado atual.</div>
            </div>""", unsafe_allow_html=True)
        ...
        with r1:
            rs = dv["Status"].fillna("Sem status").astype(str).value_counts().reset_index()
            rs.columns = ["Status","Qtd"]
            st.markdown("**Pedidos por Status**")
            st.dataframe(rs, use_container_width=True, hide_index=True, height=260)
        with r2:
            rv = dv["Venda"].fillna("Sem vendedor").astype(str).value_counts().reset_index()
            rv.columns = ["Consultor","Qtd"]
            st.markdown("**Pedidos por Consultor**")
            st.dataframe(rv, use_container_width=True, hide_index=True, height=260)
        with r3:
            rl = dv["Locadora"].fillna("Sem locadora").astype(str).value_counts().reset_index()
            rl.columns = ["Locadora","Qtd"]
            st.markdown("**Pedidos por Locadora**")
            st.dataframe(rl, use_container_width=True, hide_index=True, height=260)
        ...
        with t1:
            opcoes_ordem = [c for c in ["dateLastUpdated","Data de Inclusão","Valor Total","Mensalidade","Pedido","Venda","Status"] if c in dv_show.columns]
            ordenar_por = st.selectbox("Ordenar por", opcoes_ordem if opcoes_ordem else dv_show.columns.tolist(), key="rel_ordem_col")
        with t2:
            ordem_desc = st.checkbox("Ordem decrescente", value=True, key="rel_ordem_desc")
        with t3:
            cols_default = [c for c in ["Pedido","Locadora","Segmento","Status","Venda","Nome","Modelo Oficial","Mensalidade","Valor Total","dateLastUpdated","Placa","Chassi"] if c in dv_show.columns]
            cols_visiveis = st.multiselect("Colunas visíveis", dv_show.columns.tolist(), default=cols_default or dv_show.columns.tolist()[:10], key="rel_cols")

        if ordenar_por in dv.columns:
            serie_ord = dv[ordenar_por]
            if ordenar_por in ["Valor Total","Mensalidade","Comissão Carrera","Comissão Vendedor R$","Preço NF","KickBack","Km","Período"]:
                serie_ord = _to_num_serie(serie_ord)
            elif ordenar_por in ["dateLastUpdated","Data de Inclusão","Data de Status","Data da retirada","Vigência Final do contrato","Data Assinatura"]:
                serie_ord = pd.to_datetime(serie_ord, errors="coerce", dayfirst=True)
            dv_show = (dv_show.assign(_ord=serie_ord.values)
        ...
        st.dataframe(dv_show, use_container_width=True, height=520, hide_index=True,
            column_config={
        ...
        if "Pedido" in dv.columns and not dv.empty:
            ...
            pedido_sel = st.selectbox("Selecione um pedido", [""]+pedidos_unicos, key="rel_det_pedido")
            if pedido_sel:
                detalhe = dv[dv["Pedido"].astype(str) == str(pedido_sel)].copy()
                st.dataframe(_preparar_exibicao(detalhe), use_container_width=True, hide_index=True, height=220)

        # Download
        buf = io.BytesIO()
        _preparar_exibicao(dv).to_excel(buf, index=False, engine="openpyxl")
        buf.seek(0)
    ...
    if pode_editar:
        with abas[1]:
            ...
            for label, script, desc in scripts:
                ...
                with c1:
                    if st.button(label, key=f"run_{script}", use_container_width=True, disabled=not existe, type="primary"):
                    ...
                    with st.expander(f"📋 Log — {label}", expanded=True):
                        try:
                            ...
                            for line in proc.stdout:
                                output.append(line.rstrip()); ph.code("\n".join(output[-30:]))
                            proc.wait()
                            if proc.returncode == 0:
                                st.cache_data.clear(); st.success("✅ Concluído!")
                            else:
            ...
            st.info("🌐 Salesforce: consumido direto do Google Sheets (atualiza a cada 4h automaticamente)")
            for nome, caminho in [
    ...
    if pode_editar:
        with abas[2]:
            ...
            st.markdown("<p class='mini-note'>Para pedidos que não constam na base principal. "
                        "O consultor é preenchido automaticamente via Salesforce.</p>", unsafe_allow_html=True)
            ...
            with st.form("form_manual"):
                ...
                with m1: loc = st.selectbox("Locadora *", locadoras_opts); seg = st.selectbox("Segmento *", segmentos_opts)
                with m2: pedido = st.text_input("Nº Pedido *"); cliente = st.text_input("Cliente *")
                with m3: doc = st.text_input("CPF / CNPJ *"); status = st.selectbox("Status", status_opts)
                ...
                with l1: local_venda = st.text_input("Local de Venda")
                ...
                if pedido:
                    ...
                    st.markdown(f"<p style='font-size:13px;color:{cor_info}'>👤 Consultor: <b>{consultor_preview}</b></p>", unsafe_allow_html=True)
            ...
            if enviado:
                if not pedido or not cliente or not doc:
                    ...
                    dados = {
                        "Locadora":loc,"Segmento":seg,"Pedido":pedido,"Nome":cliente,
                        "Documento":doc,"Tipo":_tipo_cliente(doc),"Status":status,
                        ...
                        "Local de Venda":local_venda,"UF":uf,"City":city,
                        "Venda":consultor_preview,"Data de Inclusão":agora,"Origem venda":"Manual",
                    }
                    pg = st.progress(0, text="Salvando..."); pg.progress(60, text="Gravando arquivo local...")
                    if _gravar_manual_local(dados):
                        pg.progress(100, text="Concluído!")
                        _ler_manuais_local.clear(); st.cache_data.clear()
